...
bot.on('message', function (message) {
 ...
 "fields": [
  ...
  {
   "name": "!kplay (url musique youtube)",
   "value": "``Cettee commande permet de pouvoir lancer une musique youtube a partir de l'url donné !``"
  },
  {
  "name": "!khelp",
                                    ...
                                    },
                                    {
                                      ...
                                      "value": "`Cette commande permet de repondre a une question posé !`"
                                        },
                                        {
	                                      ...
	                                      {
                                               "name": "!kyoutube",
                                               "value": "`Permet de rechercher quelque chose sur youtube `"
                                              },
{
  "name": "Invite le bot !",
  "value": "https://discordapp.com/oauth2/authorize?client_id=439411792490528769&scope=bot&permissions=2105015551"
},
{
      ...
      bot.on('message', function (message) {
        ...
        bot.on('message', function (message) {
          ...
          bot.on('message', function (message) {
            ...
            bot.on('message', function (message) {
                if (message.content === 'tu suce ?') {
                  message.channel.send("Oui de 12h a 14h !")

                }
              })
              bot.on('message', function (message) {
                  if (message.content === 'son') {
                    message.channel.send("Dans une enceinte il y a ?")

                  }
                })
              bot.on('message', function (message) {
                  if (message.content === 'une membrane') {
                    message.channel.send("cool , dernière question , cité moi un magasin connu :)")
